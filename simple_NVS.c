...
//-----------------------------------------------------------------------------------------------------------------------
    ...
    {
        ESP_LOGI(Tag1, "The Storing was Successfull");
        nvs_commit(my_nvs_handle); 
    }
...
// This is how u get a int8_t value: 
    ...
    err = nvs_get_i16(my_nvs_handle, "my_16-bit", &my_value_1); // use nvs_get_i26 to get the 16-bit value
    if(err != ESP_OK)
    {
    ...
    // This is how u store a float value:
    err = nvs_get_blob(my_nvs_handle, "my_float",&my_float_0, sizeof(my_float_0)); // use nvs_set_blob to store float
    if(err != ESP_OK)
    {
    ...
    // This is how u store a double value:
    err = nvs_get_blob(my_nvs_handle, "my_double",&my_double_0, sizeof(my_double_0)); // use nvs_set_blob to store double.
    if(err != ESP_OK)
    {
    ...
    // This is how u store a char: 
    err = nvs_get_i8(my_nvs_handle, "my_char", &my_char_0); // use nvs_get_i64 to get the 64-bit value
    if(err != ESP_OK)
    {
    ...
    {
        ESP_LOGI(Tag2,"Stored string is: %s", retrieved_str);
    }
